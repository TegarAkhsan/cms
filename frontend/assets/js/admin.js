...
window.renderDashboard = async function() {
  ...
  movChart = new Chart(ctx1, {
    ...
    data: {
      ...
      datasets: [{
        label: 'Kontainer', data: (stats.monthly_data || []).map(m => m.total),
...
window.renderReports = async function() {
  ...
  rptChart = new Chart(ctx, {
    type: 'bar',
    data: {
      labels: Object.keys(byStatus).map(k => STATUS_CONFIG[k]?.label || k),
      datasets: [{
        label: 'Jumlah', data: Object.values(byStatus),
